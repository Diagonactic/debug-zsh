...
declare -x DZSH_TARGET="${1:A}"; declare -x DZSH_TARGET_DIRECTORY="${${DZSH_TARGET:h}:A}"; shift
...
pushd "$DZSH_TARGET_DIRECTORY" || __dzsh die "Cannot change to $DZSH_TARGET_DIRECTORY"
{
    "${$DZSH_TARGET:t}" "$@"
} always { popd }
